...
class VaticaninsiderSpider(scrapy.Spider):
    # 爬虫名称，唯一
    name = 'vaticaninsider'
    # 允许访问的域
    ...
    def parse(self, response):
        ...
        for url in all_urls:
            if url.startswith("/vaticaninsider"):
                # 将链接拼接全
    ...
    def parse_topic(self, response):
        ...
        for url in all_urls:
            # 找到符合这个话题的文章链接
            if re.search(r"/\d{4}/\d{2}/\d{2}/vaticaninsider/", url) is not None:
                # 将链接拼接全
                url = response.urljoin(url)
                # 检测链接是否访问过，避免 url 重复访问
                if not url in self.url_set:
    ...
    def get_Content(self, response):
        item = LastampaItem()
        # 获取文章标题
        title = response.xpath('//div[@class="ls-articoloTitolo"]/h3/text()').extract()
        if len(title) > 0:
            title = title[0]
        elif len(response.xpath('//title/text()').extract()) > 0:
            title = response.xpath('//title/text()').extract()[0]
        else:
            title = response.xpath('//script[@type="text/javascript"]/text()').extract()[0]
            title = re.search(r"var\stitolo_articolo\s=\s'(.*)'", title).group(1)
        ...
        content = ""
        # 文章内容标签
        content_text = response.xpath('//div[@class="ls-articoloTesto"]|//div[@class=ls-articoloTesto]|//div[@id="mm_description"]').extract()
